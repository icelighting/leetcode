...
class Graph(object):
    ...
    def deep_first_search(self,root = None):#递归dfs
        order = []
        def dfs(node):
        ...
        for node in self.nodes():##用以处理不连通的结点，需要重新开始，再一次dfs
    ...
    def depth_first_search2(self, root =None):#非递归 dfs
        stack = []
        order = []
        def dfs():
            while stack:
